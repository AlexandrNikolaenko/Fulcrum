...
export function BaseLink({text, href}) {
    return(
        <Link href={href} className="text-base w-fit text-white transition-all rounded-base py-[6px] px-5 bg-base-blue active:bg-dark-blue focus:bg-dark-blue hover:bg-dark-blue">{text}</Link>
    )
}

export function EditButton({action, bg}) {
        return(
        <button onClick={action} className={`flex gap-2.5 rounded-base ${bg == 'white' && 'shadow-center' } py-[6px] px-5 bg-${bg}`}>
            <span className="text-dark">Редактировать</span>
            <Image alt="Edit" src={'/Edit.svg'} width={20} height={20}/>
        </button>
    )
}
